---
name: Pseudo-Plutarch
name_greek: Ψευδοπλούταρχος
eul_aid: pyq
canonical: "https://eulogikon.org/en/pseudo-plutarch"
period: Roman
dialect: Philosophical Koine
works_count: 3
license: "https://creativecommons.org/publicdomain/mark/1.0/"
---

# Pseudo-Plutarch

*Ψευδοπλούταρχος*

**eul_aid:** `pyq` · [HTML page](https://eulogikon.org/en/pseudo-plutarch)

Pseudo-Plutarch, an unknown Roman era philosophical author. Three surviving philosophical works. Free to read and download from the Greek library archive.

## Works

- [On Music](https://github.com/eulogikon/ancient-greek-texts/blob/main/grc/pseudo-plutarch/music.md) — *περὶ Μουσικῆς* · `pyq-ac`
- [On Natural Doctrines Pleasing Philosophers](https://github.com/eulogikon/ancient-greek-texts/blob/main/grc/pseudo-plutarch/natural-doctrines.md) — *Περὶ τῶν ἀρεσκόντων τοῖς φιλοσόφοις φυσικῶν* · `pyq-aa`
- [On Rivers](https://github.com/eulogikon/ancient-greek-texts/blob/main/grc/pseudo-plutarch/rivers.md) — *Περὶ ποταμῶν* · `pyq-ab`

## License and provenance

Eulogikon publishes public-domain Greek texts in cleaned, structured, readable form and makes no copyright claim over that preparation (Public Domain Mark 1.0, https://creativecommons.org/publicdomain/mark/1.0/). Author records and the underlying corpus are at https://eulogikon.org/.

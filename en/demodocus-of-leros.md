---
name: Demodocus of Leros
name_greek: Δημόδοκος ὁ Λέριος
eul_aid: bea
canonical: "https://eulogikon.org/en/demodocus-of-leros"
period: Archaic
dialect: Ionic Elegiac
works_count: 2
license: "https://creativecommons.org/publicdomain/mark/1.0/"
---

# Demodocus of Leros

*Δημόδοκος ὁ Λέριος*

**eul_aid:** `bea` · [HTML page](https://eulogikon.org/en/demodocus-of-leros)

Demodocus of Leros, 6th century BCE Ionic elegiac poet. Two surviving poetic fragments from the Archaic period. Free to download from the Greek library archive.

## Works

- [Epigrams](https://github.com/eulogikon/ancient-greek-texts/blob/main/grc/demodocus-of-leros/epigrams.md) — *Ἐπιγράμματα* · `bea-ab`
- [Satirical Epigrams on Peoples](https://github.com/eulogikon/ancient-greek-texts/blob/main/grc/demodocus-of-leros/satirical-epigrams.md) — *Ἀποσπάσματα* · `bea-aa`

## License and provenance

Eulogikon publishes public-domain Greek texts in cleaned, structured, readable form and makes no copyright claim over that preparation (Public Domain Mark 1.0, https://creativecommons.org/publicdomain/mark/1.0/). Author records and the underlying corpus are at https://eulogikon.org/.
